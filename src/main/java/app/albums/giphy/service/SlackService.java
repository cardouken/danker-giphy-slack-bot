...
public class SlackService {
    ...
    private static final int SWITCHEROO_CHANCE = 35; // % chance of the gif not being replaced
    ...
    public SlackService(GiphyClient giphyClient, RestTemplate slackRestTemplate) {
        ...
        this.slack = Slack.getInstance().methods(BOT_TOKEN);
    ...
    private void postEphemeralMessage(List<LayoutBlock> layoutBlocks, String channel, String userId) {
        try {
            slack.chatPostEphemeral(req -> req
                    .channel(channel)
                    .user(userId)
                    .text("sample text")
